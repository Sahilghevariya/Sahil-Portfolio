...
export default function Contact() {
  ...
  const sendViaMailto = (payload) => {
    try {
      const to = "sahilghevariya001@gmail.com";
      const subjectText = payload.subject || `Message from ${payload.from_name || "Website Visitor"}`;
      const body = `Name: ${payload.from_name || ''}\nContact: ${payload.contact_info || ''}\n\n${payload.message || ''}`;
      const mailto = `mailto:${to}?subject=${encodeURIComponent(subjectText)}&body=${encodeURIComponent(body)}`;
  ...
  const handleSubmit = (e) => {
    ...
    if (!EMAILJS_SERVICE_ID || !EMAILJS_TEMPLATE_ID || !EMAILJS_PUBLIC_KEY) {
      ...
      setStatus("❌ Email service not configured. Opening your mail client as a fallback...");
      // open mail client fallback
      sendViaMailto({ from_name: form.name, contact_info: form.contact, subject: form.subject, message: form.message });
      return;
    }
    ...
    emailjs
      ...
      .then(
        ...
        (error) => {
          ...
          // If the error indicates an invalid public key or similar auth issue, fallback to mailto
          const text = (error && (error.text || error.message || '')).toString();
          if (/public key/i.test(text) || /invalid key/i.test(text) || /403/.test(text)) {
            setStatus("❌ Failed to send: The EmailJS public key appears invalid. Opening your mail client as a fallback...");
            sendViaMailto({ from_name: form.name, contact_info: form.contact, subject: form.subject, message: form.message });
            return;
          }
  ...
  const quickLinks = [
    ...
    { img: gmailLogo, title: "Email", link: "mailto:sahilghevariya001@gmail.com" },
